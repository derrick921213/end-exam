...
BPlusTreeNode *deserialize_tree(FILE *file) {
    ...
    node->keys = (unsigned long *)malloc(node->n * sizeof(unsigned long));
    if (!node->keys) {
        perror("Failed to allocate memory for keys");
        free(node);
        exit(EXIT_FAILURE);
    }
    fread(node->keys, sizeof(unsigned long), node->n, file);

    if (!node->is_leaf) {
        node->children = (BPlusTreeNode **)malloc((node->n + 1) * sizeof(BPlusTreeNode *));
        if (!node->children) {
            perror("Failed to allocate memory for children");
            free(node->keys);
            free(node);
